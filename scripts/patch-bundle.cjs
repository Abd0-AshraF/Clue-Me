...
function ThemeTransitionOverlay({theme}){
  ...
  return u.jsxs("div",{
    className:"fixed inset-0 z-[100000] pointer-events-none flex flex-col items-center justify-center overflow-hidden animate-fadeOut",
...
function SetupWizardModal({open:n,onClose:r}){
  ...
  const themes=[
    {id:"mot",nameAr:"الغموض والتحقيق 🕵️‍♂️",nameEn:"Mystery Mot 🕵️‍♂️",descAr:"ثيم اللعبة الافتراضي الغامض والداكن",descEn:"Original detective theme",color:"from-zinc-900 to-zinc-950 border-red-500"},
...
// 9. Patch ci (Dialog Modal) to lock background scrolling on open and restore on close
const oldCi = 'function ci({open:n,onClose:r,title:s,closeLabel:i,children:l,className:f}){const p=T.useId(),m=T.useRef(null);return T.useEffect(()=>{if(!n)return;const h=g=>{g.key==="Escape"&&r()};return document.addEventListener("keydown",h),typeof window<"u"&&window.matchMedia&&window.matchMedia("(pointer: fine)").matches&&m.current?.focus(),()=>document.removeEventListener("keydown",h)},[n])';

const newCi = 'function ci({open:n,onClose:r,title:s,closeLabel:i,children:l,className:f}){const p=T.useId(),m=T.useRef(null);return T.useEffect(()=>{if(!n)return;const h=g=>{g.key==="Escape"&&r()};const origO=document.body.style.overflow,origH=document.body.style.height;document.body.style.overflow="hidden";document.body.style.height="100vh";return document.addEventListener("keydown",h),typeof window<"u"&&window.matchMedia&&window.matchMedia("(pointer: fine)").matches&&m.current?.focus(),()=>{document.removeEventListener("keydown",h);document.body.style.overflow=origO;document.body.style.height=origH;}},[n])';

safeReplace('ci Dialog scroll-locking patch', oldCi, newCi);

// 10. Patch _b (Room Not Found/Expired screen) to clear clue-me:last-session to break the redirect loop
const old_b = 'function _b({code:n}){const{t:r}=Ve();return ld(`${r.routes.notFoundTitle} \\u2014 CLUE ME`),u.jsx("div",{className:"flex min-h-dvh items-center justify-center p-4",children:u.jsxs(cn,{className:"w-full max-w-md p-6 text-center",children:[u.jsx("h1",{className:"text-xl font-bold text-ink",children:r.routes.notFoundTitle}),u.jsx("p",{className:"mt-2 text-sm text-ink-soft",children:zt(r.routes.notFoundBody,{code:n??"\\u2014"})}),u.jsxs("div",{className:"mt-5 flex flex-col gap-2",children:[u.jsx(Je,{size:"lg",onClick:()=>Sn("/create"),children:r.actions.createRoom}),u.jsx(Je,{variant:"secondary",onClick:()=>Sn("/join"),children:r.actions.joinRoom}),u.jsx(Je,{variant:"ghost",onClick:()=>Sn("/"),children:r.routes.home})]})]})})}';

const new_b = 'function _b({code:n}){T.useEffect(()=>{try{localStorage.removeItem("clue-me:last-session");}catch(e){}},[]);const{t:r}=Ve();return ld(`${r.routes.notFoundTitle} \\u2014 CLUE ME`),u.jsx("div",{className:"flex min-h-dvh items-center justify-center p-4",children:u.jsxs(cn,{className:"w-full max-w-md p-6 text-center",children:[u.jsx("h1",{className:"text-xl font-bold text-ink",children:r.routes.notFoundTitle}),u.jsx("p",{className:"mt-2 text-sm text-ink-soft",children:zt(r.routes.notFoundBody,{code:n??"\\u2014"})}),u.jsxs("div",{className:"mt-5 flex flex-col gap-2",children:[u.jsx(Je,{size:"lg",onClick:()=>Sn("/create"),children:r.actions.createRoom}),u.jsx(Je,{variant:"secondary",onClick:()=>Sn("/join"),children:r.actions.joinRoom}),u.jsx(Je,{variant:"ghost",onClick:()=>Sn("/"),children:r.routes.home})]})]})})}';
...
// 12. Support "mot" theme in ThemeProvider (f2)
const oldThemeD2 = 'if(n==="light"||n==="dark"||n==="mani"||n==="mani-dark"||n==="system")return n';
const newThemeD2 = 'if(n==="light"||n==="dark"||n==="mani"||n==="mani-dark"||n==="system"||n==="mot")return n';
safeReplace('d2 theme support for mot', oldThemeD2, newThemeD2);

const oldThemeWh = 'const wh=["light","dark","mani","mani-dark"];';
const newThemeWh = 'const wh=["light","dark","mani","mani-dark","mot"];';
safeReplace('wh theme list support for mot', oldThemeWh, newThemeWh);

const oldThemeEffect = 'T.useEffect(()=>{const h=document.documentElement,g=f==="mani"||f==="mani-dark";h.classList.toggle("dark",f==="dark"||f==="mani-dark"),h.classList.toggle("mani",g),h.classList.toggle("mani-dark",f==="mani-dark");try{localStorage.setItem(U1,r)}catch{}},[f,r]);';
const newThemeEffect = 'T.useEffect(()=>{const h=document.documentElement,g=f==="mani"||f==="mani-dark";h.classList.toggle("dark",f==="dark"||f==="mani-dark"||f==="mot"),h.classList.toggle("mani",g),h.classList.toggle("mani-dark",f==="mani-dark"),h.classList.toggle("mot",f==="mot");try{localStorage.setItem(U1,r)}catch{}},[f,r]);';
safeReplace('theme effect toggling for mot', oldThemeEffect, newThemeEffect);
